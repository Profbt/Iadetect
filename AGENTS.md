...
- **`js/detector.js`** — análise pura (sem DOM): `AI_PATTERNS_PT` (padrões curados para pt-BR) + `AI_PATTERNS_EN` (fallback, id prefixado `en-`, peso ×0.5) combinados em `AI_PATTERNS`, `detectAI` (retorna `stylisticIndex`, alias do `score`), métricas estilísticas via `tokenizeWords`/`splitSentences` (Intl.Segmenter com fallback regex; `computeBurstiness`, `computeLexicalDiversity`, `computeAvgSentenceLength`, `computeParagraphUniformity`), `METRIC_WEIGHTS`/`PATTERN_WEIGHT`, marcadores U+XXXX, `CHAR_INFO`/`getCharInfo`, regex (`INVISIBLE_REGEX`), `buildBackdropHTML`. A maior parte é dados + funções puras.
- **`js/cleaner.js`** — `cleanText` (fluxo `html`/`invisible`/`decodeMarkers`), `cleanInvisiblesPreservingEmoji` (conservadora; emojis/ZWJ preservados, `U+2028/2029` → quebras; agressiva via `sanitizeAggressive`), `looksLikeHTML` (doctype + ≥2 blocos + ≥3 inline), `extractTextFromHTML` (DOMParser), `decodeHTMLEntities` (nomeadas + numéricas, sem DOM), `normalizeTypography`, `buildDiffHTML` (o HTML do diff; `renderDiff` só injeta em `#diffBody`).
- **`js/app.js`** — o resto: estado global, helpers de DOM (`$`, `getOpts`, `isTextSufficient`, `updateCounts`, `syncBackdrop`), renderização (dashboard de índice com breakdown/métricas via `renderScoreDashboard(detection, sourceText)`, auditoria, painel de resultado em 2 seções — documento tratado e texto limpo — com `showResultPanel`/`showResultTextPanel`/`downloadTextFile`), extração/limpeza de arquivos (Office/PDF/imagem), provedores de reescrita, análise comparativa e todos os **event listeners**.
...
6. **Testar antes de terminar**: valide a sintaxe com `node --check js/detector.js` (idem para os outros), rode `node test/run-tests.js` (suíte Node, precisa de `Intl.Segmenter`) e `node test/browser-test.js` (suíte headless Chrome — exige `chrome.exe`; DOM/HTML parsing só roda no navegador, `DOMParser` não existe no Node). Sem `util.inspect` nos testes: derruba o harness.
...
`handleFile` (drop/upload) → extrai texto → `setInputText` → `runAnalyze`: `detectAI` (score/evidências/breakdown) + `cleanText` (saída) + `renderAudit` (chars suspeitos) + `renderDiff` (original→limpo) + stats. O resultado usa `renderScoreDashboard` (gauge + barras por categoria `#breakdownBars` + cards de métricas `#metricsGrid`) — métricas só até 200 KB (perf), `#scoreBreakdown`/`#metricsGrid` ocultos sem dados. Texto curto (<100 palavras ou <500 caracteres) não recebe índice: `isTextSufficient` → `#scoreInsufficient` visível e gauge oculto.

## Índice de padrões e comparativo (análise comparativa)

- Índice (0–100) é heurístico e sem sabotar texto limpo: soma de padrões (PT com peso cheio; EN com peso ×0.5, id `en-`) `PATTERN_WEIGHT=80` + `METRIC_WEIGHTS` (burstiness 8, diversidade 6, uniformidade 4, comprimento 2 — soma 100). Penalidade por métrica = peso × (valor que não é "saudável"); comprimento só penaliza se média > 25. Ajuste aqui se calibrar em corpus real.
- Vereditos das métricas: burstiness ≥0.6 alta / ≥0.35 média; diversidade ≥0.6 alta / ≥0.45 média; uniformidade ≥0.75 alta / ≥0.5 média (uniformidade ALTA é suspeita — invertida); comprimento <12 curta, 12–20 média, 20–30 longa, >30 muito longa (informativo). Texto curto retorna neutro (0.5/média) — **não** inflar TTR de texto curto.
...
- Painel `#comparePanel` (antes de `.rewrite-panel`): é **colapsável** — estado inicial fechado, persistido em `localStorage` (`cleanmark_compare_open`, gerenciado por `setCompareOpen`). Auto-expande ao usar `btnSendA`/`btnSendB` (copiam `#input`/`#output`), ao colar texto relevante nos campos e no fluxo pós-reescrita. `runCompare()` sempre garante painel aberto; lado abaixo do mínimo (via `isTextSufficient`) mostra badge "Texto insuficiente", não recebe `data-score` e a comparação segue parcial (resumo avisa). Monta tabela de métricas com delta (Δ verde = melhora) via `buildCompareTable`, diff e resumo. `updateCompareBadge()` mantém o `#compareToggleBadge` (mini-resumo `A: x → B: y (±z%)`, verde se B melhorou). Guarda de rewrite: `fillCompareFromRewrite` preenche original × reescrita, compara e rola até o painel — não quebrar esse fluxo.
- Cores de categoria fixas no dashboard: `CAT_COLORS` em `app.js` (Vocabulário `#c084fc`, Frases `#fbbf24`, Estrutura `#f87171`). Classes de veredito de métricas: `verdict-ok/-warn/-bad/-muted`. Índices A/B usam `[data-verdict]` (`scoreVerdict`: clean/low/medium/high); deltas da tabela usam `td.delta.good/.bad/.neutral`.
- Painel de resultado (`showResultPanel`) mostra duas seções: **Documento tratado** (conteúdo original preservado + `#btnDownloadClean`) e **Texto limpo** (saída do `cleanText`/reescrita + `#btnDownloadText` via `downloadTextFile`, nome `texto_limpo.txt`).
...
node test/run-tests.js            # suíte Node (padrões PT/EN, métricas, invisíveis, HTML/entidades)
node test/browser-test.js         # suíte headless Chrome (DOM, DOMParser, fluxo da UI, rótulos)
npx serve .
```

## Vocabulário da UI (não regredir)

- UI em pt-BR e sem rotulagem probabilística: usar **"Índice de padrões"**/**"Índice"** (nunca "Score IA"/"sinal de IA"), **"Padrões de IA"** (nunca "Sinais de IA"), **"Padrões atípicos detectados"**, vereditos "Aparece em ~9 de 10 textos com padrões típicos de IA". Comportamento antigo sai do DOM (testado em `browser-test.js`). Identificadores internos `rawScore`/`score` ficam como estão.